...
| 摄取管道（Ingest Pipeline） | 节点内轻量字段转换           | 重型解析会增加管道执行节点的处理器负载   |
...
负责执行这些处理器的 Elasticsearch 节点称为**摄取节点（ingest node）**。摄取节点不是一种独立软件，而是带有 `ingest` 角色的普通 Elasticsearch 节点；同一个节点可以同时承担数据、主节点候选和摄取等多个角色。使用摄取管道时，集群中至少要有一个节点具备 `ingest` 角色。未显式设置 `node.roles` 时，默认角色中包含 `ingest`，所以本课程的单节点实验环境不需要额外部署摄取节点。

可以用节点信息接口查看每个节点的角色；响应中的 `roles` 数组包含 `ingest`，就表示该节点能够执行摄取管道：

```http
GET /_nodes?filter_path=nodes.*.name,nodes.*.roles
```

轻量摄取负载通常可以与其他角色共用节点。摄取管道数量多、处理逻辑复杂或写入量很大时，才需要通过压测评估专用摄取节点；专用摄取节点主要承担 `ingest` 角色，以免管道处理争用主节点或数据节点的处理器资源。

摄取管道适合处理靠近写入端的轻量、确定性转换，例如统一大小写、转换日期和数值类型、删除无用字段。它不负责从外部系统采集数据，也不适合耗时较长的复杂计算，因此不能完全替代 Elastic Agent、Logstash、Kafka 消费者等接入组件。
...
第 07 课已经说明，数值字段的 `coerce` 默认为 `true`，所以 `scaled_float` 映射本身通常也
能把 `"399.00"` 解析为可索引的数值。这里仍显式使用 `convert`，并不是因为
`scaled_float` 完全不能接收数值字符串，而是为了在映射解析之前统一文档格式：转换成功后
普通 `_source.price` 也是 JSON 数值，而不再保留为字符串；转换失败时还可以通过
`on_failure` 记录明确的摄取错误并执行后续处理。映射层 `coerce` 主要保证字段能够建立
数值索引，不等同于完整的数据清洗和错误路由。
